...
func StatsView(state StatsViewState, width, height int) string {
	...
	subtitle := fmt.Sprintf("Sorted by: %s | Tab to change | V to toggle videos | Backspace to exit", sortNames[state.SortColumn])
	...
	// Data rows
	for i := state.ScrollOffset; i < len(state.Stats) && i < state.ScrollOffset+visibleHeight; i++ {
		stat := state.Stats[i]
